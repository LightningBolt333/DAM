...
interface AIAssistant {
    ...
    val maxTokens: Int
        get() = properties.getProperty("max_tokens")?.toIntOrNull() ?: 512
    ...
    suspend fun analyzeSentiment(text: String): String {
        val strictPrompt = """
        You are a sentiment analysis API. You must respond with ONLY a JSON object, nothing else.
        No explanations, no markdown, no code blocks, no preamble.
        
        Analyze the sentiment of this text on a scale of 1-7:
        1=Very Negative, 2=Negative, 3=Slightly Negative, 4=Neutral,
        5=Slightly Positive, 6=Positive, 7=Very Positive
        
        TEXT: "$text"
        
        Example of the EXACT format you must follow:
        {"rating": 6, "justification": "The text expresses happiness about the weather."}
        
        Now analyze the TEXT above and respond with ONLY the JSON object:
    """.trimIndent()

        val raw = apiCallWithBackoff(strictPrompt)
